...
public class PersistenceTest {
    ...
    public void consultaProductoPorFechaDespacho(){
        ...
        ********************************************************************/
        Date d = new Date(2015-1900,8,12);
        int[] idFacturas = {f1.getIdFacturas(),f2.getIdFacturas(),f3.getIdFacturas(),f4.getIdFacturas(),f5.getIdFacturas()};
        Despacho ddd = (Despacho)session.load(Despacho.class, 7);
        List<ProductoEnVenta> productosFechaDespacho = PersistenceFacade.productosPorFechaDespacho(session, idFacturas, d);
        Set<ProductoEnVenta> setProductos = new HashSet<ProductoEnVenta>(productosFechaDespacho);
        List<ProductoEnVenta> productosPrueba = new ArrayList<ProductoEnVenta>(0);
        
        productosPrueba.add(pev1);
        productosPrueba.add(pev3);
        productosPrueba.add(pev3);
        
        
        assertEquals("La cantiad de productos a despachar es incorrecta", productosFechaDespacho.size(), productosPrueba.size());
        for (ProductoEnVenta productosPruebaX : productosPrueba) {
            assertTrue(setProductos.contains(productosPruebaX));
        }
